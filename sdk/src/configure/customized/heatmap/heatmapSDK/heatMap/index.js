...
function getPoints() {
    ...
    var success = function(data) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            var ele_x = list[i].$element_x
            var ele_y = list[i].$element_y
            var ele_path = list[i].$element_path

            if (Util.paramType(ele_path) !== 'String' || Util.paramType(ele_x) !== 'Number' && Util.paramType(ele_y) !== 'Number') {
                continue;
            }

            var ele_porint = parseEvent(ele_path)
            if (!ele_porint) {
                continue
            }
            var ele_postion = elementPostion(ele_porint)
            if (ele_postion.hidden == true) {
                continue
            }

            var point = {
                x: ele_postion.left + ele_x,
                y: ele_postion.top + ele_y,
                value: 1
            };
            if (ele_x > ele_porint.offsetWidth) {
                point.x = ele_postion.left + Math.abs(ele_postion.left - x)
            }
            if (ele_y > ele_porint.offsetHeight) {
                point.y = ele_postion.top + Math.abs(ele_postion.top - y)
            }
        ...
        for (var i = 0; i < points.length; i++) {
            for (var z = 0; z < points.length; z++) {
                if (z !== i && radiusPoint(points[i], points[z])) {
                    points[i].value += 1
                }
                max = Math.max(points[i].value, max)
            }
        }
...
function rightColorRange(e) {
    ...
    function upRightBtn(e) {
        ...
        heat.heatObj.setDataMax(showMapConfig.max * rightDataMax / 100)
        headBtnMap.rightBtn.releaseCapture && rightBtn.releaseCapture();
