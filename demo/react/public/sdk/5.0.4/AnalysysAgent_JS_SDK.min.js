(function (global, factory) {
    ...
    typeof arkDefine === 'function' && arkDefine.amd ? arkDefine(factory) :
    (global = global || self, global.AnalysysAgent = factory());
    ...
    ***************************************************************************** */
    ...
    var publicAttrs = ['$lib', '$lib_version', '$platform', '$is_login', '$debug'];
    // 通用属性，行为事件都会上报
    ...
    // utm相关属性
    '$utm_campaign_id', '$utm_campaign', '$utm_medium', '$utm_source', '$utm_content', '$utm_term'];
    // element相关属性
    ...
    // 预制事件列表与事件属性
    var events = {
        $startup: __spreadArrays(commonAttrs, ['$is_first_time' //首次访问，只在startUp
        ]),
        $end: __spreadArrays(commonAttrs, ['$duration' //使用时长
        ]),
        $pageview: __spreadArrays(commonAttrs, ['$url_domain', '$referrer', '$referrer_domain', '$startup_time' // 此行下面5个只在 pageView 中有
        ]),
        page_close: __spreadArrays(commonAttrs, ['$referrer', 'pagestaytime']),
        $user_click: __spreadArrays(elementAttrs, commonAttrs),
        $web_click: __spreadArrays(commonAttrs, elementAttrs, ['$page_width', '$page_height', '$click_x', '$click_y', '$element_x', '$element_y', '$element_clickable']),
        $webstay: __spreadArrays(commonAttrs, ['$referrer', '$referrer_domain', '$viewport_width', '$viewport_position', '$viewport_height', '$event_duration', '$device_type']),
        track: __spreadArrays(commonAttrs),
        $profile_set_once: [firstVisitTime, '$first_visit_language'],
        $alias: ['$original_id'],
        $getPresetProperties: __spreadArrays(publicAttrs, commonAttrs, [firstVisitTime])
    };
     ...
     * @returns
     */
    function booleanCheck(value, key) {
        var res = isBoolean(value);
        if (!res) {
            errLog(key, value, 60003);
        }
        return res;
    }
    function functionCheck(value, key) {
        var res = isFunction(value);
        if (!res) {
            errLog(key, value, 60004);
        }
        return res;
    }
    function numberCheck(value, key) {
        var res = isNumber(value);
        if (!res) {
            errLog(key, value, 60002);
        }
        return res;
    }
    function stringCheck(value, key) {
        var res = isString(value);
        if (!res) {
            errLog(key, value, 60001);
        }
        return res;
    }
    function objectCheck(value, key) {
    ...
    */
    ...
    //# sourceMappingURL=isSpider.js.map
    ...
    var IndexedDb = /** @class */function () {
        ...
        IndexedDb.prototype.getObjectStore = function () {
        ...
        IndexedDb.prototype.add = function (data, successFn, errorFn) {
            var request = this.getObjectStore().add(data);
            request.onsuccess = function (event) {
        ...
        IndexedDb.prototype.delete = function () {
            var request = this.getObjectStore().delete(1);
            request.onsuccess = function (event) {};
        };
        // 更新数据
        IndexedDb.prototype.put = function (data, successFn, errorFn) {
            var request = this.getObjectStore().put(data);
            request.onsuccess = function (event) {
                successFn && successFn();
            };
            request.onerror = function (event) {
    ...
    var cookieKey = domainUrl ? storageKey + "." + domainUrl : '';
    var storeDb = new IndexedDb();
    var dBhasData = false;
    function getStorage(fn) {
        var cookieStore = function () {
            return cookieKey ? getCookie(cookieKey) : null;
        };
        var store = getLocalStorage() || cookieStore() || null;
        if (!globalWindow.indexedDB) {
            ...
            var get = function () {
                storeDb.get(function (res) {
                    dBhasData = !!res;
                    var content = res ? res.content : store;
    ...
    */
    ...
    //# sourceMappingURL=startUpTime.js.map
    ...
    function getSessionId() {
        ...
        var offset_GMT = date.getTimezoneOffset();
        var getGmt = function (time) {
            return new Date(time + offset_GMT * 60 * 1000 + 8 * 60 * 60 * 1000).getDate();
        };
        var sessionDate = core.SEESIONDATE;
    ...
    // sdk初始化前钩子是否执行完成
    var beforeInitReady = false;
    var implementBeforeInit = function (fn) {
        function next() {
            if (!beforeInitReady) {
        ...
        var beforeInit = config.beforeInit;
        if (beforeInit) {
            var f = beforeInit.call(globalWindow.AnalysysAgent, __assign({}, config), next);
    ...
    // hybrid模式下上报事件
    function callNative(functionName, functionParams, isCallback) {
        var obj = {
        ...
        // ios
        if (globalWindow.webkit && globalWindow.webkit.messageHandlers && globalWindow.webkit.messageHandlers.AnalysysAgent && globalWindow.webkit.messageHandlers.AnalysysAgent.postMessage) {
            globalWindow.webkit.messageHandlers.AnalysysAgent.postMessage(obj);
        }
        // 安卓
        if (globalWindow.AnalysysAgentHybrid && globalWindow.AnalysysAgentHybrid.analysysHybridCallNative) {
            globalWindow.AnalysysAgentHybrid.analysysHybridCallNative(JSON.stringify(obj));
        }
    }
    // hybrid模式下回调类方法
    function callNativeCallback(functionName, functionParams, fn) {
        if (fn) {
    ...
    function setValidKey(optionData, url) {
        ...
        var uploadURL = config.uploadURL[config.uploadURL.length - 1] !== '/' ? config.uploadURL : config.uploadURL.substring(0, config.uploadURL.length - 1);
     ...
     */
    function sendData(data, fn, isTrack) {
        var xwhat = data.xwhat;
        // Hybrid模式下由原生端上报
        if (isHybrid && ['$web_click', '$webstay', '$user_click'].indexOf(xwhat) === -1) {
            // hybrid模式下删除这些属性，由原生上报
            var arr = ['$is_first_day', '$session_id', '$is_time_calibrated', '$startup_time', '$lib', '$lib_version', '$platform', '$debug', '$is_login'];
            arr.forEach(function (o) {
                delete data.xcontext[o];
            });
            var functionParams = [data.xcontext];
            if (xwhat === '$pageview') {
            ...
            callNative(isTrack ? 'track' : xwhat, functionParams);
            return;
        }
        // 设置回调函数
        if (fn && isFunction(fn)) {
        ...
        // 上报之前执行
        emit('afterSend', __assign(__assign({}, data), { xcontext: __assign({}, data.xcontext) }));
        // 页面卸载时采用beacon上报
    ...
    function pageView(pageName, properties, fn) {
        ...
        var beforePageView = config.beforePageView;
        if (beforePageView) {
            var obj = __assign(__assign({}, res), { xcontext: __assign({}, res.xcontext) });
            // 设置属性
            var setAttrs = function (attrs) {
                var obj = attrCheck(attrs, '$pageview');
    ...
    function pageClose() {
        ...
        function getHideTime() {
            var pageCloseAttr = eventAttribute.pageClose;
            var hideTime = pageCloseAttr.hideTime;
            return !pageCloseAttr.hideStartTime ? hideTime : hideTime + (res.xwhen - pageCloseAttr.hideStartTime);
        }
        // 获取访问时长，排除小于0情况
        var pagestaytime = res.xwhen - eventAttribute.pageview.xwhen - getHideTime();
        var attrs = {
            pagestaytime: pagestaytime > 0 ? pagestaytime : res.xwhen - eventAttribute.pageview.xwhen
        };
        // 填充当前pv上报的title
        var title = eventAttribute.pageview['$title'];
        if (title) {
            attrs['$title'] = title;
        }
        // 合并通用属性 // 绑定页面属性
        ...
        var beforePageClose = config.beforePageClose;
        if (beforePageClose) {
            var obj = __assign(__assign({}, res), { xcontext: __assign({}, res.xcontext) });
            // 设置属性
            var setAttrs = function (attrs) {
                var obj = attrCheck(attrs, 'page_close');
                res.xcontext = assign({}, res.xcontext, obj);
                return res.xcontext;
            };
    ...
    //# sourceMappingURL=pageclose.js.map
    ...
    //# sourceMappingURL=userClick.js.map
    ...
    //# sourceMappingURL=webClick.js.map
    ...
    //# sourceMappingURL=webstay.js.map
    ...
    */
    ...
    function track(eventName, eventAttrs, fn) {
        ...
        // 获取上报数据模块
        var res = fillData('track');
        var trackAttrs = eventAttrs && !isFunction(eventAttrs) ? attrCheck(eventAttrs, eventName) : {};
        // 增加使用时长属性
        ...
        var beforeTrack = config.beforeTrack;
        if (beforeTrack) {
            // 设置属性
            var setAttrs = function (attrs) {
                var obj = attrCheck(attrs, eventName);
                res.xcontext = assign({}, res.xcontext, obj);
                return res.xcontext;
            };
            var obj = __assign(__assign({}, res), { xcontext: __assign({}, res.xcontext) });
    ...
    function nativeCallback(name, params) {
        var nativeCallbackNam = nativeCallbackFn[name];
        if (nativeCallbackNam) {
            nativeCallbackNam.forEach(function (o) {
                o(params);
            });
            nativeCallbackFn[name] = [];
        }
    }
    //# sourceMappingURL=nativeCallback.js.map
    ...
    function on(eventName, fn) {
        eventMap$1[eventName] ? eventMap$1[eventName].push(fn) : eventMap$1[eventName] = [fn];
    }
    function emit(eventName) {
        ...
        if (eventMap$1[eventName]) {
            eventMap$1[eventName].forEach(function (o) {
                o.apply(void 0, args);
            });
        }
    }
    ...
    function setConfig(options, fn) {
        ...
        optionArr.forEach(function (o) {
            ...
            if (rule && rule.ck.length) {
                var value = options[o];
                var isOk = true;
                for (var i = 0; i < rule.ck.length; i++) {
                    var fn_1 = rule.ck[i];
                    if (!fn_1(value, o)) {
                        isOk = false;
                        break;
        ...
        function procedure() {
            if (isReady()) {
                ...
                autoTrigger();
                // 之心生命周期事件监听
                emit('afterInit', __assign({}, config));
                fn && fn(config);
            }
        }
        // 客户端程序是否准备就绪
    ...
    var ArkJsSdk = /** @class */function () {
        function ArkJsSdk() {
            ...
            this.getSuperProperties = ready(getSuperProperties);
            this.unRegisterSuperProperty = ready(unRegisterSuperProperty);
            this.clearSuperProperties = ready(clearSuperProperties);
            this.profileSetOnce = ready(profileSetOnce);
            this.profileSet = ready(profileSet);
            this.profileAppend = ready(profileAppend);
            this.profileIncrement = ready(profileIncrement);
            this.profileDelete = ready(profileDelete);
            this.profileUnset = ready(profileUnset);
            this.reset = ready(reset);
            this.track = ready(track);
            this.timeEvent = timeEvent;
            this.alias = ready(alias);
            this.getPresetProperties = ready(getPresetProperties);
            this.identify = ready(identify);
            this.getDistinctId = ready(getDistinctId);
            this.pageProperty = ready(pageProperty$1);
